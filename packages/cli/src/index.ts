      ...
      if (options.dryRun) {
        ...
        for (const vuln of fixable) {
          try {
            ...
            // Apply config-level fixes: redact hardcoded secrets with env var placeholders
            if (vuln.type === 'EXPOSED_API_KEY' && vuln.location?.path) {
              const serverConfig =
                (config as Record<string, MCPServerConfig>)[vuln.server] ||
              ...
              if (locationPath.startsWith('env.') && serverConfig.env) {
                ...
                if (serverConfig.env[envKey]) {
                  serverConfig.env[envKey] = `\${${envKey.toUpperCase()}}`;
                  applied = true;
                }
              } else if (locationPath === 'auth.token' && serverConfig.auth) {
                (serverConfig.auth as any).token = '${AUTH_TOKEN}';
                applied = true;
              } else if (
                locationPath === 'auth.credentials.password' &&
                serverConfig.auth?.credentials
              ) {
                (serverConfig.auth.credentials as any).password = '${AUTH_PASSWORD}';
                applied = true;
              } else if (locationPath === 'oauth.clientSecret' && serverConfig.oauth) {
                (serverConfig.oauth as any).clientSecret = '${OAUTH_CLIENT_SECRET}';
                applied = true;
              }
            }

            if (applied) {
              console.log(chalk.green(`  ✅ Fixed: ${vuln.title}`));
            } else {
              console.log(chalk.yellow(`  ⚠️  ${vuln.title} — manual remediation required`));
              if (vuln.remediation?.commands) {
                vuln.remediation.commands.forEach((cmd) => {
                  console.log(chalk.gray(`    ${cmd}`));
                });
        ...
        fixSpinner.succeed(`Processed ${fixedCount} fixes`);
